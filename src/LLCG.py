...
def run(proc_id,
    ...
    if proc_id == 0:
        ...
        g_train_global, reverse_eids_global = to_bidirected_with_reverse_mapping(train_graph)
    ...
    if model_name == 'GraphSAGE':
        ...
        model_global = GraphSAGE(in_size, 256).to(device)
    elif model_name == 'GCN':
        model = GCN(in_size, 256).to(device)
        model_global = GCN(in_size, 256).to(device)
    elif model_name == 'GAT':
        model = GAT(in_size, 256).to(device)
        model_global = GAT(in_size, 256).to(device)
    elif model_name == 'GATv2':
        model = GATv2(in_size, 256).to(device)
        model_global = GATv2(in_size, 256).to(device)
    ...
    opt_global = torch.optim.Adam(model_global.parameters(), lr=lr)
    ...
    for epoch in range(0, epochs):
        ...
        torch.distributed.barrier()
        model_global.load_state_dict(model.state_dict())

        if proc_id == 0:
            # global correction
            if model_name == 'GraphSAGE':
                sampler_global = NeighborSampler([25, 10, 5])
            else:
                sampler_global = MultiLayerFullNeighborSampler(3)
            sampler_global = as_edge_prediction_sampler(
                sampler_global,
                exclude="reverse_id",
                reverse_eids=reverse_eids_global,
                negative_sampler=negative_sampler.Uniform(1),
            )
            dataloader_global = DataLoader(
                g_train_global,
                torch.arange(g_train_global.num_edges()),
                sampler_global,
                device=device,
                batch_size=batch_size,
                shuffle=True,
            ...
            model_global.train()
            for iter, (input_nodes, pair_graph, neg_pair_graph, blocks) in enumerate(dataloader_global):
                if iter < 1:
                    x = blocks[0].srcdata["feat"]
                    pos_score, neg_score = model_global(pair_graph, neg_pair_graph, blocks, x, None)
                    score = torch.cat([pos_score, neg_score])
                    pos_label = torch.ones_like(pos_score)
                    neg_label = torch.zeros_like(neg_score)
                    labels = torch.cat([pos_label, neg_label])
                    loss_global = F.binary_cross_entropy_with_logits(score, labels)
            ...
            if (epoch+1)%eval_steps == 0:
                val_accs = test(device, g_train_global, model, val_edges, batch_size, acc)
                test_accs = test(device, g_train_global, model, test_edges, batch_size, acc)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='LLCG')
    ...
    output_path = path + '/output/partitions-' + str(number_partition) + '/' + dataset + '/METIS/'
